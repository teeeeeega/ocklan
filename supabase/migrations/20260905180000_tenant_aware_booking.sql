...
begin
  if exists (
    ...
    raise exception 'BOOKING_MIGRATION_BLOCKED: appointments contains rows with NULL coach_id';
  end if;
end;
$$;
...
create index if not exists appointments_coach_schedule_idx
  on public.appointments (coach_id, starts_at, ends_at);

create index if not exists availability_rules_coach_weekday_active_idx
  on public.availability_rules (coach_id, weekday, is_active);

create index if not exists blocked_times_coach_schedule_idx
  on public.blocked_times (coach_id, starts_at, ends_at);
